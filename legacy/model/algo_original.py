...
def exp_dec_error(y_true, y_pred):
    return K.exp(
        - 5. * K.sqrt(K.clip(K.sum(K.square(y_pred), axis=-1, keepdims=True), 0.000001, 10)))  # tend to increase y_pred
...
def loss_diff_s_s_(y_true, y_pred):
    return K.square(
        1. - K.sqrt(K.clip(K.sum(y_pred, axis=-1, keepdims=True), 0.000001, 1.)))  # tend to increase y_pred --> loss -1
...
class CRAR(LearningAlgo):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_norm=0, freeze_interval=1000,
        ...
        self.learn_and_plan = neural_network(self._batch_size, self._input_dimensions, self._n_actions,
        ...
        self.learn_and_plan_target = neural_network(self._batch_size, self._input_dimensions, self._n_actions,
                                                    self._random_state, high_int_dim=self._high_int_dim,
                                                    internal_dim=self._internal_dim)
        ...
        self.full_Q_target.compile(optimizer='rmsprop',
                                   loss='mse')  # The parameters do not matter since training is done on self.full_Q
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        self.loss_disambiguate1 += self.encoder.train_on_batch(states_val, np.zeros_like(
            Es))  # np.zeros((self._batch_size,self.learn_and_plan.internal_dim)))
        ...
        if (self.update_counter % 500 == 0):
            ...
            print(self.loss_T / 500., self.lossR / 500., self.loss_gamma / 500., self.loss_Q / 500.,
    ...
    def qValues_planning_abstr(self, state_abstr_val, R, gamma, T, Q, d, branching_factor=None):
        ...
        if (d == 0):
            ...
            return r_vals_d0 + gamma_vals_d0 * np.amax(
                self.qValues_planning_abstr(next_x_predicted, R, gamma, T, Q, d=d - 1,
                                            branching_factor=branching_factor).reshape(
                    len(state_abstr_val) * this_branching_factor, branching_factor[0]), axis=1).flatten()
    ...
    def chooseBestAction(self, state, mode, *args, **kwargs):
        ...
        q_vals = self.qValues_planning([np.expand_dims(s, axis=0) for s in copy_state], self.R, self.gamma,
                                       self.transition, self.Q, d=di[mode])
    ...
    def _compile(self):
        ...
        if (self._update_rule == "sgd"):
            ...
            optimizer1 = SGD(lr=self._lr, momentum=self._momentum, nesterov=False,
                             clipnorm=self._clip_norm)  # Different optimizers for each network;
            optimizer3 = SGD(lr=self._lr, momentum=self._momentum, nesterov=False,
                             clipnorm=self._clip_norm)  # to possibly modify them separately
            ...
            optimizer1 = RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon,
                                 clipnorm=self._clip_norm)  # Different optimizers for each network;
            optimizer3 = RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon,
                                 clipnorm=self._clip_norm)  # to possibly modify them separately
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.diff_s_s_.optimizer.lr,
                    self._lr / 5.)  # /5. for simple laby or simple catcher; /1. for distrib of laby
